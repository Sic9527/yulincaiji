...
//            console.log("No prompt");
        ...
        if (_A["icon_show"]) {
            _e = ' <i class="iconfont icon-cuowu"></i>';
            _s = ' <i class="iconfont icon-zhengque"></i>';
            if (_A["icon_img"]) {
        ...
        code = function () {
            _o.attr("valiid", +_id);
            // console.log(_o);//当前input对象
            // $("body").prepend('<label class="validate_label_prompt" id="valiid' + _id + '"></label>');
            _o.parent().append('<em class="" id="valiid' + _id + '"></em>');
            // $("#valiid" + _id).css({
        ...
        input_blur = function (id) {
            ...
            _o.blur(function () {
                ...
                if (err_validate(validate, prompt) == false) {
                    _o.addClass('errorTips');//输入错误时添加错误样式
                    mui.alert(_o.attr('data-labelNames')+'输入有误');
...
//          p.html(_prompt["min"].replace("｛0｝", num));
        ...
        _.each(function () {
			...
			$('#pserson_sbumit').click(function(){		
				debugger;
				console.log('人员提交')
            	$('#tableName_val').val($(this).attr('data-tableName'));
            	var lbdsj = $('#lbssj').val();
            	var sfzyxq = $('#sfzyxq').val();
            	var zzzqfrq = $('#zzzqfrq').val();
            	$('#sj').val(lbdsj);
            	$('#yxq').val(sfzyxq);
            	$('#qfrq').val(zzzqfrq);
            	/*alert($('#tableName_val').val($(this).attr('data-tableName')));*/
            	var gr_tel = $('input[name="col_15"]').val();
				var re = /^[1][3,4,5,7,8][0-9]{9}$/;
			 	if (!re.test(gr_tel)) {
			  		mui.alert("请输入正确手机号码！");
			  		return false;
			 	}
            	console.log('tableId_val:'+$('#tableName_val').val());
            	console.log(JSON.stringify($("#"+$(this).attr('dataType')).serialize()));
                _validate.each(function () {
                    $(this).blur();
                    if (_err != 0) {
                        return false;
				...
				mui.ajax({ 
					url:base_url+"base/add/"+$('#tableName_val').val(),
					type:'post', 
					dataType:'json',//服务器返回json格式数据
					async:false,
					data:$("#personFormData").serialize(),
            ...
            $('#car_submit').click(function(){
				...
				console.log('车辆提交')
            	$('#tableName_val').val($(this).attr('data-tableName'));
            	var gr_cartel = $('input[name="col_8"]').val();
            	var gr_driverid = $('input[name="col_9"]').val();
            	var imgsrc = $('#photo_img_val').val();
            		$('#photo_imgcar_val').val(imgsrc);
			 	var reg = /^\d{6}(18|19|20)?\d{2}(0[1-9]|1[12])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/;
				var re = /^[1][3,4,5,7,8][0-9]{9}$/;
			 	if (!re.test(gr_cartel)) {
			  		mui.alert("驾驶人手机号输入有误！");
			  		return false;
			 	}
			 	var reg = /^\d{6}(18|19|20)?\d{2}(0[1-9]|1[12])(0[1-9]|[12]\d|3[01])\d{3}(\d|X)$/;
			 	if (!reg.test(gr_driverid)) {
			  		mui.alert("驾驶证号输入有误");
			  		return false;
			 	}
            	console.log('tableId_val:'+$('#tableName_val').val());
...
//					console.log(data); 
            ...
            //转向采集数据提交
            $('#submitBtnZhuanxiang').click(function(){		
				console.log('转向采集数据提交');
            	/*$('#tableName_val').val($(this).attr('data-tableName'));*/
            	console.log('tableId_val:'+$('#tableName_val').val());
            	console.log($('#tableName_val').val());
            	console.log(JSON.stringify($("#"+$(this).attr('dataType')).serialize()));
                _validate.each(function () {
                    $(this).blur();
                    if (_err != 0) {
